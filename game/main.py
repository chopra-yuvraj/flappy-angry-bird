...
class Pipe:
    ...
    def draw(self, scr):
        ...
        pygame.draw.line(scr, PIPE_HL, (ix + 8, 0), (ix + 8, self.height - ch), 3)
        ...
        pygame.draw.line(scr, PIPE_HL, (ix + 8, self.height + PIPE_GAP + ch), (ix + 8, SCREEN_HEIGHT - GROUND_HEIGHT), 3)
...
class Game:
    def __init__(self):
        ...
        self.sfont = pygame.font.Font(None, 64)
        self.smfont = pygame.font.Font(None, 26)
        # Sky gradient
        ...
        self.clouds = [{'x': random.randint(0, SCREEN_WIDTH + 100), 'y': random.randint(20, 220),
                        'sp': random.uniform(0.3, 1.0), 'sz': random.uniform(0.7, 1.3)} for _ in range(6)]
        self.best = 0
        self.particles = []
        self.shake = 0
        self.goff = 0
    ...
    def update_pipes(self):
        ...
        for p in self.pipes:
            ...
            if not p.scored and p.x + PIPE_WIDTH < self.bird.x:
                ...
                self.score += 1
                for _ in range(8):
                    self.particles.append(Particle(SCREEN_WIDTH // 2, 70, SCORE_GOLD))
    ...
    def draw_bg(self):
        ...
        for c in self.clouds:
            c['x'] -= c['sp']
            if c['x'] < -120:
                c['x'] = SCREEN_WIDTH + random.randint(50, 150)
                c['y'] = random.randint(20, 220)
            sz = c['sz']
            cx, cy = int(c['x']), int(c['y'])
            pygame.draw.circle(self.screen, WHITE, (cx, cy), int(25 * sz))
            pygame.draw.circle(self.screen, WHITE, (cx - int(18 * sz), cy + int(8 * sz)), int(20 * sz))
            pygame.draw.circle(self.screen, WHITE, (cx + int(18 * sz), cy + int(8 * sz)), int(20 * sz))
            pygame.draw.circle(self.screen, WHITE, (cx + int(8 * sz), cy - int(6 * sz)), int(16 * sz))
        gy = SCREEN_HEIGHT - GROUND_HEIGHT
        pygame.draw.rect(self.screen, GROUND_BRN, (0, gy, SCREEN_WIDTH, GROUND_HEIGHT))
        pygame.draw.rect(self.screen, GROUND_GRN, (0, gy, SCREEN_WIDTH, 12))
        ...
        for i in range(-24 + off, SCREEN_WIDTH + 24, 24):
            pygame.draw.line(self.screen, GROUND_DK, (i, gy + 14), (i - 12, gy + GROUND_HEIGHT), 1)
    ...
    def draw_ui(self):
        ...
        if not self.game_started and not self.game_over:
            ...
            ty = SCREEN_HEIGHT // 2 - 60 + int(math.sin(tk / 500) * 5)
            ...
            self.screen.blit(st, st.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 20)))
            if self.best > 0:
        ...
        if self.game_over:
            ...
            py = (SCREEN_HEIGHT - ph) // 2 - 20
            pn = pygame.Rect(px, py, pw, ph)
            pygame.draw.rect(self.screen, (45, 55, 72), pn, border_radius=12)
            pygame.draw.rect(self.screen, SCORE_GOLD, pn, 3, border_radius=12)
            go = self.tfont.render("GAME OVER", True, (220, 60, 60))
            self.screen.blit(go, go.get_rect(center=(SCREEN_WIDTH // 2, py + 35)))
            pygame.draw.line(self.screen, (80, 90, 110), (px + 20, py + 60), (px + pw - 20, py + 60), 2)
            sc = self.font.render(f"Score:  {self.score}", True, WHITE)
            self.screen.blit(sc, (px + 30, py + 78))
            nb = self.score >= self.best and self.score > 0
            bc = SCORE_GOLD if nb else (180, 180, 190)
            bs = self.font.render(f"Best:   {self.best}", True, bc)
            self.screen.blit(bs, (px + 30, py + 115))
            if nb:
                nt = self.smfont.render("NEW!", True, SCORE_GOLD)
                self.screen.blit(nt, (px + pw - 60, py + 118))
            mc = None
            if self.score >= 30: mc = MEDAL_G
            elif self.score >= 15: mc = MEDAL_S
            elif self.score >= 5: mc = MEDAL_B
            if mc:
                mx, my = px + pw - 55, py + 95
                pygame.draw.circle(self.screen, mc, (mx, my), 18)
                pygame.draw.circle(self.screen, BLACK, (mx, my), 18, 2)
            pygame.draw.line(self.screen, (80, 90, 110), (px + 20, py + 155), (px + pw - 20, py + 155), 2)
            tk = pygame.time.get_ticks()
            pu = 0.6 + 0.4 * math.sin(tk / 350)
            rc = tuple(int(c * pu) for c in (200, 220, 255))
            rt = self.font.render("Press R or Tap", True, rc)
            self.screen.blit(rt, rt.get_rect(center=(SCREEN_WIDTH // 2, py + ph - 35)))
